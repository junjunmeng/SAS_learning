...
* tabs found in an Excel spreadsheet. See the book Implementing
* CDISC Using SAS Second Edition for details. Version 20190406.
*
* PARAMETERS:
...
data codelists;
  length codedvalue translated $ 200;
  set codelists;

  format codedvalue translated;
  informat codedvalue translated;
  codedvalue = htmlencode(codedvalue);
  translated = htmlencode(translated);
run;

data compmethod;
  length computationmethod $ 3000;
  set compmethod;

  format computationmethod;
  informat computationmethod;
...
%if "&standard" = "ADAM" %then
  %do;
    data analysisresults;
      length programmingcode docuementation purpose $ 2000 reason paramlist $ 200 displayid refleafid $ 40;
      set analysisresults;
      where displayid ne '';
      
      format reason paramlist purpose documentation programmingcode;
      informat reason paramlist purpose documentation programmingcode;
    
      arrow + 1;
      selectioncriteria = htmlencode(selectioncriteria); 
      paramlist = htmlencode(paramlist);
      reason = htmlencode(reason); 
      purpose = htmlencode(purpose); 
...
data itemgroupdef;
  set itemgroupdef;
    ...
    ** Trim all trailing blanks and other non-visible characters to ensure no warn ings are issued by ;
    ** the stylesheet;
      ...
      do;
        ...
        **   set/domain for the predecessor;
        ...
        if index(origin,'.')=0 then
          put @11 '<TranslatedText xml:lang="en">' origin +(-1) '.' variable +(-1) '</TranslatedText>' ;
        else
          put @11 '<TranslatedText xml:lang="en">' origin +(-1) '</TranslatedText>' ;
        put @9  '</Description>' 
            ;
      end;
    else
...
data codelists enumerated;	
    ...
    **** DERIVE WHETHER A CODELIST ITEM IS EXTENDED;
    if codelistcode ne '' and codelistitemcode='' then
      extended = 'Yes';

    **** DERIVE WHETHER A CODELIST IS ENUMERATED LIST ONLY;
    retain enumerated;
    if first.codelistname then
      enumerated = 'Y';
    if codedvalue ne translated and translated ne '' then
      enumerated = 'N';
...
data codelists;
  merge codelists(drop=enumerated)
        enumerated(keep=codelistname enumerated);
  by codelistname;
run;

proc sort
    data=codelists;
    by codelistname ordernumber; 	
run;
...
data codelists;
    ...
    **** output codelists that are simple enumerated lists;
    if enumerated = 'Y' then
      do;
        put @7  '<EnumeratedItem CodedValue="' codedvalue +(-1) '"' @;
        if extended = 'Yes' then
          put ' def:ExtendedValue="Yes"' @;
        if rank ne . then
          put ' Rank="' rank +(-1) '"' @;
        if ordernumber ne . then
          put ' OrderNumber="' ordernumber +(-1) '"' @;
        if codelistitemcode ne '' then 
          put '>' / @9 '<Alias Context="nci:ExtCodeID" Name="' codelistitemcode +(-1) '"/>'; 
        else
          put '>'; 
        put @7 '</EnumeratedItem>';
      end;

    **** output codelists that have translations;
    if enumerated = 'N' then
	 do;
    ...
    **** output codelists that are pointers to external codelists;
    ...
    if last.codelistname then
      do;
        if codelistcode ne '' then
          put @7 '<Alias Context="nci:ExtCodeID" Name="' codelistcode +(-1) '"/>';
        put @5 '</CodeList>';
      end;
run;
